...
private function sqlRNA($variavel, $altura, $conexao, $limite){
		$sql = "SELECT ".$variavel." FROM sodar_atual WHERE z<".$altura." AND ".$variavel." < ".$limite." ORDER BY data DESC, z DESC LIMIT 1";
		//echo $sql."<br/>";
	...
	if($linha['z'] < 210){			
		//if($linha['z']==30){
			...
			if($linha['speed'] < 99.99){
				...
				#consulta valor observado mais pŕoximo
				$ret = $this->sqlRNA('speed',$linha['z'],$conexao,'99.99');
				array_push($entrada, $ret);
				$flag1 = 1;
				if(empty($ret)){$flag2 = 1;}				
				array_push($variavel_faltante, '('.$linha['z']." : speed)");
			}
			##Direcao
			if($linha['dir'] < 999){
				...
				$flag1 = 1;
				if(empty($ret)){$flag2 = 1;}
				array_push($variavel_faltante, '('.$linha['z']." : dir)");
			}
			##TI
			if($linha['TI'] < 99.99){
				...
				$flag1 = 1;
				if(empty($ret)){$flag2 = 1;}
				array_push($variavel_faltante, '('.$linha['z']." : TI)");
			}
			##TKE
			if($linha['TKE'] < 99.99){
				...
				$flag1 = 1;
				if(empty($ret)){$flag2 = 1;}
				array_push($variavel_faltante, '('.$linha['z']." : TKE)");
			}
			##EDR
			if($linha['EDR'] < 99.99){array_push($entrada,$linha['EDR']);}
				...
				$flag1 = 1;
				if(empty($ret)){$flag2 = 1;}
				array_push($variavel_faltante, '('.$linha['z']." : EDR)");
			}
...
file_put_contents("/var/www/html/SDPVA/RNA/entrada/entrada_altitude.txt", implode(" ", $entrada));
//file_put_contents("/var/www/html/SDPVA/RNA/entrada/entrada_superficie.txt", implode(" ", $entrada_sup));

if($flag2==1){$flag_variavel=2;}
elseif($flag1==1){$flag_variavel=1;}

$sql = "INSERT INTO entrada_RNA VALUES(DEFAULT,'".$data."',".implode(",", $entrada).",".$flag_variavel.",'".implode(";",$variavel_faltante)."')";
file_put_contents("/var/www/html/SDPVA/RNA/entrada/sql_entrada_RNA.txt",$sql);
$conexao->query($sql);
...
file_put_contents("/var/www/html/SDPVA/RNA/entrada/entrada_ws.txt", implode(" ", $entrada_ws));
