...
cur.execute("SELECT s.id, s.data, s.z, s.speed, s.dir, s.U, s.V from sodar_atual s where s.data > DATE_SUB(NOW(), INTERVAL 375 MINUTE) AND s.data < NOW() AND s.z <=520 order by s.data DESC, s.z DESC")
...
x = 25 ## Número de observações com intervalo de 15 minutos para 24 horas 
a = 50 ## Número de alturas , indo de 30 a 520 temos 50 alturas
y = 25 ## Numero de observacoes com intervalo de 15 minutos para 24 horas 
...
ax.text(-2.0, altitude1*0.305, nome_altitude1, rotation=30, fontsize=9, color='#551A8B')
ax.text(-2.0, altitude2*0.305 , nome_altitude2, rotation=30, fontsize=9, color='#551A8B')
ax.text(-2.0, altitude3*0.305 , nome_altitude3, rotation=30, fontsize=9, color='#551A8B')
ax.text(-2.0, altitude4*0.305 , nome_altitude4, rotation=30, fontsize=9, color='#551A8B')
...
fig.savefig(diretorio_graficos+'barbela6h.png', bbox_inches='tight')
