...
##SELECT s.id, s.data, s.z, s.speed, s.dir from sodar_atual s where s.data > DATE_SUB(NOW(), INTERVAL 1455 MINUTE) AND s.data < NOW() AND s.z <=520 order by s.data DESC, s.z DESC

## Escrevendo SQL's
##cur.execute("SELECT * FROM sodar_atual order by id desc limit 1250")
...
cur.execute("SELECT s.id, s.data, s.z, s.speed, s.dir, s.U, s.V from sodar_atual s where s.data > DATE_SUB(NOW(), INTERVAL 735 MINUTE) AND s.data < NOW() AND s.z <=520 order by s.data DESC, s.z DESC")
##cur2.execute("SELECT DISTINCT DATE_FORMAT(s.data,'%H:%i') FROM sodar_atual s ORDER BY id DESC LIMIT 24")
...
x = 49 ## Número de observações com intervalo de 15 minutos para 24 horas 
a = 50 ## Número de alturas , indo de 30 a 520 temos 50 alturas
y = 49 ## Numero de observacoes com intervalo de 15 minutos para 24 horas 
z = 530 ## Altura máxima acrescida de 10 unidades
...
ax.text(-2.9, altitude1*0.305, nome_altitude1, rotation=30, fontsize=9, color='#551A8B')
ax.text(-2.9, altitude2*0.305 , nome_altitude2, rotation=30, fontsize=9, color='#551A8B')
ax.text(-2.9, altitude3*0.305 , nome_altitude3, rotation=30, fontsize=9, color='#551A8B')
ax.text(-2.9, altitude4*0.305 , nome_altitude4, rotation=30, fontsize=9, color='#551A8B')
...
fig.savefig(diretorio_graficos+'vetor12h.png', bbox_inches='tight')
