...
##cur.execute("SELECT * FROM sodar_atual order by id desc limit 650")
cur.execute("SELECT s.id, s.data, s.z, s.speed, s.dir, s.U, s.V from sodar_atual s where s.data > DATE_SUB(NOW(), INTERVAL 195 MINUTE) AND s.data < NOW() AND s.z <=520 order by s.data DESC, s.z DESC")
...
x = 13 ## Número de observações com intervalo de 15 minutos para 3 horas 
a = 50 ## Número de alturas , indo de 30 a 520 temos 50 alturas
y = 13 ## Numero de observacoes com intervalo de 15 minutos para 3 horas 
...
fig.savefig(diretorio_graficos+'barbela3h.png', bbox_inches='tight')
